...
#pragma mark - 纹理属性单独一个数组
- (void)renderNew2 {
    //顶点数据，前三个是顶点坐标， 中间四个是顶点颜色
    GLfloat attrArr[] = {
...
#warning 这里要思考一个问题,纹理属性单独设置的话,那么应该绑定哪个缓冲区?还是直接使用?
    GLuint texIBO;
    glGenBuffers(1, &texIBO);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, texIBO);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(texCoord), texCoord, GL_STATIC_DRAW);
    ...
    //加载纹理
    NSString* filePath = [[NSBundle mainBundle] pathForResource:@"texture.jpg" ofType:@""];
