...
esPerspective ( ESMatrix *result, float fovy, float aspect, float nearZ, float farZ )
{
   ...
   frustumH = tanf ( fovy / 360.0f * PI ) * nearZ;  //计算裁剪窗口的高度
...
void ESUTIL_API
//创建一个平行视景体(就是一个长方形空间区域,也相当于立体空间进行3D裁剪).
//这种投影意味着离观察者较远的对象上看上去不会变小(与透视投影相反)
//实际上这个函数的操作是创建一个正射投影矩阵,并且用这个矩阵诚意当前矩阵

//注意,所有的near和far同时为正或者同时为负,值不能相同.
//如果没有其他变换，正射投影的方向平行于Z轴，且视点朝向Z负轴.
//这意味着物体在视点前面时far和near都为负值，物体在视点后面时far和near都为正值,也就是z的正方向是不是对着你的脸
//只有在视景体里的物体才能显示出来。如果最后两个值是(0，0)，也就是near和far值相同了，视景体深度没有了，整个视景体都被压成个平面了，就会显示不正确。
esOrtho (ESMatrix *result,  //当前矩阵
         float left,        //x 最小值
         float right,       //x 最大值
         float bottom,      //y 最小值
         float top,         //y 最大值
