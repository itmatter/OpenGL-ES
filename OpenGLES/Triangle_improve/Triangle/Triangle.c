...
void DrawTriangleWithVAO ( ESContext *esContext ) {
    ...
    // 更新uniform颜色
    // 这里面简单做一个渐变的效果,因为glfwGetTime这写函数只能在window中使用,这里面简单做个其他功能来简单处理一下.
    GLint vertexColorLocation = glGetUniformLocation(userData->programObject, "cusColor");
    glUniform4f(vertexColorLocation, 0.0f, sin(timeValue), 0.0f, 1.0f);
